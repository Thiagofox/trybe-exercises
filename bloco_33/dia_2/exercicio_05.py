...
Implemente um gerenciador de quartos de hotel, simplificado, conforme o
diagrama a seguir.
...
Atributos:
    ...
    cpf - CPF do hospede (para fins de simplificação, não é necessário validar
    o CPF).
...
Atributos:
    ...
    isponível, altera o estado do quarto e cria uma reserva para cada hospede.
    ...
    que um quarto esteja disponível, além de respeitar a capacidade, ele
    não pode estar reservado.

Observação: considere todos os atributos como sendo públicos para fins
de simplificação da implementação.

Dica: Você pode utilizar o método sorted com o parâmetro key , para definir
seu critério de ordenação. E para utilizar um atributo como critério você
pode utilizar a função attrgetter do módulo operator .
"""
...
class Quarto:
    def __init__(
        self, numero, andar, quantidade_de_hospedes, preco, reservado=False
    ):
        self.numero = numero
        self.andar = andar
...
class Hotel:
    ...
    def check_out(self, quarto):
        ...
        self.reservas = [
            reserva for reserva in self.reservas if reserva.quarto != quarto
        ]
    ...
    def quartos_disponiveis(self, quantidade_de_hospedes):
        return sorted(
            [
                ...
                and quantidade_de_hospedes <= quarto.quantidade_de_hospedes
            ],
